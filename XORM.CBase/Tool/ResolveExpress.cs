...
namespace XORM.CBase.Tool
{
    ...
    public class ResolveExpress
    {
        ...
        public string GetExpressionRightFiled(Expression exp)
        {
            ...
            {
                throw new Exception("不是有效拉姆达格式" + exp.ToString());
            }
        }
        public void ResolveExpression(ResolveExpress re, Expression exp)
        {
            ...
            if (isContainsNot && expStr.Match(@" => Not\(.+?\)") && !expStr.Contains("Contains"))
            {
        ...
        private StringBuilder SqlWhereBuilder = new StringBuilder();
        ...
        private string AddParas(ref string left, string right)
        {
            ...
            if (right == null)
            {
                this.SqlParams.Add(DBNull.Value);
            }
            else
            {
                this.SqlParams.Add(right);
            }
            return oldLeft;
        }
